...
# ---------------------------------------------------------------------------
# Section 5 – IsolationForest
# ---------------------------------------------------------------------------

def _make_iforest_data():
    rng = np.random.default_rng(42)
    inliers  = rng.standard_normal((200, 4))
    outliers = rng.standard_normal((20, 4)) + 10.0
    return inliers, outliers


def test_iforest_basic_fit_predict():
    inliers, outliers = _make_iforest_data()
    X_train = np.vstack([inliers, outliers])
    clf = IsolationForest(n_estimators=50, contamination=0.1).fit(X_train)

    pred_outliers = to_list(clf.predict(outliers))
    pred_inliers  = to_list(clf.predict(inliers))

    assert all(p == -1.0 for p in pred_outliers), "Outliers should be labelled -1"
    n_inlier_correct = sum(1 for p in pred_inliers if p == 1.0)
    assert n_inlier_correct > len(inliers) * 0.7, "Most inliers should be labelled +1"


def test_iforest_score_samples_ordering():
    inliers, outliers = _make_iforest_data()
    X_train = np.vstack([inliers, outliers])
    clf = IsolationForest(n_estimators=50, contamination=0.1).fit(X_train)

    outlier_scores = np.array(to_list(clf.score_samples(outliers)))
    inlier_scores  = np.array(to_list(clf.score_samples(inliers)))

    assert outlier_scores.mean() < inlier_scores.mean(), (
        "Outliers should have lower anomaly scores than inliers (more anomalous)"
    )


def test_iforest_decision_function_equals_score_samples():
    inliers, outliers = _make_iforest_data()
    X_train = np.vstack([inliers, outliers])
    clf = IsolationForest(n_estimators=20, contamination=0.1).fit(X_train)

    X_test = inliers[:10]
    scores_a = np.array(to_list(clf.score_samples(X_test)))
    scores_b = np.array(to_list(clf.decision_function(X_test)))
...
def test_iforest_score_before_fit_raises():
    clf = IsolationForest()
    with pytest.raises(ValueError, match="not fitted"):
        clf.score_samples(np.ones((5, 4)))


def test_iforest_contamination_zero_raises():
    with pytest.raises(ValueError):
        IsolationForest(contamination=0.0)


def test_iforest_contamination_above_half_raises():
    with pytest.raises(ValueError):
        IsolationForest(contamination=0.51)


def test_iforest_contamination_half_valid():
    clf = IsolationForest(contamination=0.5)
    assert clf.contamination == 0.5


def test_iforest_max_samples_clamps_to_n():
    """max_samples > n_samples should be silently clamped (line 75 of isolation_forest.py)."""
    rng = np.random.default_rng(5)
    X = rng.standard_normal((50, 4))
    clf = IsolationForest(n_estimators=10, max_samples=1000, contamination=0.1).fit(X)
    pred = to_list(clf.predict(X))
    assert len(pred) == 50
